...
const addFavorite = () => {
    ...
    const fillHeart = (e) => {
        ...
        if (target.matches('.book__favorite')) {
            if (target.classList.contains('active')) {
                fetch('/site/remove-from-favorite?id=' + target.dataset.id ).then(() => {
                    target.classList.remove('active')
                })
            }else{

                fetch('/site/add-to-favorite?id=' + target.dataset.id ).then(() => {
    ...
    if (slider){
        slider.addEventListener('click', fillHeart);
...
const hideBookTitle = () => {
    ...
    bookTitles.forEach(title => {
        if (title.textContent.length > 17) {
            title.textContent = title.textContent.trim().substr(0, 16) + '...'
        }
    })
